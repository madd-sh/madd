...
description: This skill should be used when decomposing a contract into fractions for incremental development, planning the order of fraction execution, or managing fraction boundaries. Use when the user mentions "fraction", "decompose tasks", "plan fractions", "split work", or when the conductor needs to break a contract into independently deliverable units.
metadata:
  short-description: Fraction decomposition algorithm
---
...
A **fraction** is an independently deliverable unit of work — a subset of contract tasks that can go through a complete maker->ci->breaker cycle without depending on unfinished tasks from other fractions.
...
The conductor invokes fraction planning **after architect completes**, before the first maker invocation.
...
## Fraction Context for Maker/Breaker Handoff

When the conductor invokes maker/breaker for a specific fraction, it should include:
...
This scoped context prevents context saturation by limiting what each maker/breaker invocation needs to process.
